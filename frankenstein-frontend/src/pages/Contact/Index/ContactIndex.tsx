...
const ContactIndex = () => {
  ...
  const fetchContacts = async (page: number = 1, search: string | null = null) => {
    try {
      let response = await fetch(`http://localhost:3000/api/v1/contact?limit=5&order=DESC${page ? `&page=${page}` : ""}${search ? `&search=${search}` : ""}`, {
        credentials: "include",
      });
                          ...
                          <tr key={id}>
                            <td className={classes}>
                              <div className="flex items-center gap-3">
                                <Avatar
                                  src={contactImage ? `${imageURL}${contactImage}` : "../../../../public/assets/Images/user.png"}
                                  alt={name}
                                  size="sm"
                                  placeholder={undefined}
                                  onPointerEnterCapture={undefined}
